...
function Home() {
  ...
  useEffect(() => {
    if (user) {
      const savedTrips = localStorage.getItem(`vehicleTrips_${user.id}`);
      if (savedTrips) setTrips(JSON.parse(savedTrips));
    }
  }, [user]);

  // Save trips to localStorage (user-specific)
  useEffect(() => {
    if (user) {
      localStorage.setItem(`vehicleTrips_${user.id}`, JSON.stringify(trips));
    }
  }, [trips, user]);
